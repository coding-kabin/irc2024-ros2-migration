...
if __name__ == '__main__':
    rclpy.init()
    node = rclpy.create_node('ld_sensor_subscriber')

    # Create publishers for sensor_msg and atmos_msg messages
    sensor_pub = node.create_publisher(sensor_msg, queue_size=10, 'sensor_topic')
    atmos_pub = node.create_publisher(atmos_msg, queue_size=30, 'atmos_topic')

    # Subscribe to ld_sensor_data topic
    node.create_subscription(_Int16MultiArray, 'ld_sensor_data', ld_sensor_callback)
