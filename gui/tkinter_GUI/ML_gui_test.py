...
def video_publisher():
    ...
    ml_pub = node.create_publisher(String, queue_size=1, "/ml")
    # Video file path
...
if __name__ == '__main__':
    rclpy.init()
    node = rclpy.create_node('video_publisher', anonymous=True)
    image_pub = node.create_publisher(Image, queue_size=1, '/camera/image_raw')
    bool_pub = node.create_publisher(Int8, queue_size=1, '/bool_topic')
